...
const ProfilePage = () => {
  ...
  const {error, loading, token,user} = useSelector((state) => state.auth);
  ...
  const handleProfileSubmit = async (e) => {
    ...
    try {
      await updateProfile(newFormData, token);
  ...
  const handleAddressSubmit = async (e) => {
    ...
    try {
        if (selectedAddress) {
            await toast.promise(
              updateAddress(selectedAddress.addressId, addressFormData, token),
              {
            ...
            await toast.promise(
              createAddress(addressFormData, token),
              {
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl p-8 w-1/2 max-w-xl transition-all duration-300">
            <form onSubmit={handleAddressSubmit}>
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className="text-gray-700 dark:text-gray-300">Zip code</label>
                  <input
                    type="text"
